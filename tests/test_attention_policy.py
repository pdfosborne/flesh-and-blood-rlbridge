...
def test_attention_gradient_flow() -> None:
    ...
    assert model.text_embed_table.grad is None


def test_predict_policy_value_numpy() -> None:
    model = _AttentionPolicyValueV2(16, d_model=32, n_layers=1, n_heads=4, seed=2)
...
def test_state_dict_json_round_trip() -> None:
    model = _AttentionPolicyValueV2(8, d_model=32, n_layers=1, n_heads=4, seed=3)
    other = _AttentionPolicyValueV2(8, d_model=32, n_layers=1, n_heads=4, seed=3)
