...
def run(
    *,
    ...
    if not fe_only:
        ...
        from fab_bridge.talishar_env import _format_env_line, training_env_path  # noqa: PLC0415
        from flesh_and_blood_rlbridge.talishar_backend_pool import (  # noqa: PLC0415
            reconcile_reserved_shard_urls,
        )

        training_env_values = reconcile_reserved_shard_urls(
            training_env_values,
            healthy_training_urls=backend_urls,
            assets_path=talishar_assets_dir(),
        )
        path = training_env_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        lines = [
            "# Auto-generated by start_talishar.py — do not edit while stacks are running.",
            _format_env_line("TALISHAR_URL", training_env_values["TALISHAR_URL"]),
            _format_env_line("TALISHAR_FE_URL", training_env_values["TALISHAR_FE_URL"]),
        ]
        if "TALISHAR_URLS" in training_env_values:
            lines.append(_format_env_line("TALISHAR_URLS", training_env_values["TALISHAR_URLS"]))
        if "TALISHAR_EVAL_URL" in training_env_values:
            lines.append(
                _format_env_line("TALISHAR_EVAL_URL", training_env_values["TALISHAR_EVAL_URL"])
            )
        if "TALISHAR_RENDER_URL" in training_env_values:
            lines.append(
                _format_env_line("TALISHAR_RENDER_URL", training_env_values["TALISHAR_RENDER_URL"])
            )
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
