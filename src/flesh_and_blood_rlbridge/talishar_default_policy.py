...
# Built once at module load — avoids recreating the dict on every action lookup.
...
def _is_defense_window(state: dict[str, Any], legal_actions: list[dict[str, Any]]) -> bool:
    ...
    return any("block" in _normalize(a.get("label", "")) for a in legal_actions)
...
def choose_talishar_action_index(
    ...
    # Build zone cache once for the whole call — avoids repeated state.get() lookups.
    zone_cache: dict[str, list] = {
        zone: state.get(state_key, [])
        for zone, state_key in _ZONE_KEY_MAP.items()
    }

    # Pre-normalize all action labels once.
    ...
    for i, action in enumerate(legal_actions):
        if _to_int(action.get("action_code", 0)) == 99 or any(
            t in labels[i] for t in _PASS_LABELS
        ):
    ...
    yes_like = [
        i for i in non_pass
        if zones[i] in {"button", "popup"} and any(t in labels[i] for t in _YES_LABELS)
    ]
    if yes_like:
        return yes_like[0]
    ...
    if defensive:
        ...
        for i in non_pass:
            ...
            block_value = float(_estimate_defense(action, state, card))
            if block_value <= 0:
                continue
            defense_scores[i] = (
                block_value,
                -float(_estimate_attack(action, state, card)),
            )
