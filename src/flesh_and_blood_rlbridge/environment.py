...
class FleshAndBloodEnvironment(rlbridgeEnvironment):
    ...
    def _opponent_turn(self) -> None:
        ...
        if idx < 0 or idx >= len(opp.hand):
            self._last_event = "Opponent passed (invalid policy action)"
            self._end_turn(1)
            self._start_turn(0)
            return

        best = self._cards[opp.hand[idx]]
    ...
    def _opponent_auto_block(self) -> None:
        ...
        defender = self._players[1]
        try:
            best_idx = self._opponent_policy.select_block_index(self)
        except Exception:
            self._opponent_type = "preset_logic"
            self._opponent_policy = self._build_opponent_policy(self._opponent_type)
            best_idx = self._opponent_policy.select_block_index(self)
        if best_idx is not None:
            if best_idx < 0 or best_idx >= len(defender.hand):
                return
