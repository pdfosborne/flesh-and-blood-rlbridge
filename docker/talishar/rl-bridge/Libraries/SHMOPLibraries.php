...
/**
 * SHMOPLibraries.php (rl-bridge overlay)
 *
 * Same as Talishar/Libraries/SHMOPLibraries.php, except WriteGamestateCache's
 * resize-and-write path and ReadGamestateCache's read path are guarded by a
 * per-game flock(). Upstream deletes and recreates the shmop segment (to grow
 * it) with no locking: a concurrent reader/writer for the same game can hold
 * an open handle to a segment that gets deleted out from under it mid-op,
 * which segfaults the PHP shmop extension under load (observed as repeated
 * Apache child crashes on the checkpoint-eval shard, which polls both seats
 * back-to-back far more than a single paced training shard).
 *
 * Installed by docker-compose overlay — do not edit Talishar/ upstream.
 */
...
function _rlBridgeGamestateCacheLockPath($key)
{
  return rtrim(sys_get_temp_dir(), "/") . "/fab_shm_gs_" . $key . ".lock";
}

/**
 * Acquire a lock guarding the shmop segment identified by GamestateID($name).
 * Returns the lock file handle (caller must fclose it, which also releases
 * the flock) or null if the lock file could not be opened.
 */
function _rlBridgeLockGamestateCache($gameName, $exclusive)
{
  $key = GamestateID($gameName);
  $path = _rlBridgeGamestateCacheLockPath($key);
  $handle = @fopen($path, "c");
  if ($handle === false) {
    return null;
  }
  flock($handle, $exclusive ? LOCK_EX : LOCK_SH);
  return $handle;
}
...
function WriteGamestateCache($name, $data)
{
  ...
  $lock = _rlBridgeLockGamestateCache($name, true);
  try {
    $size = 32768;
    $seq = 0;
...
{
  $key = GamestateID($name);
  $lock = _rlBridgeLockGamestateCache($name, false);
  try {
